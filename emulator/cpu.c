...
int Emulate8080Op(State8080 *state)
{
    ...
    case 0x07: // RLC : A = A << 1; bit 0 = prev bit 7; CY = prev bit 7 (rotate left)
    {
        uint8_t x = state->a;
        state->a = ((x & 0x80) >> 7) | (x << 1); // left shift by 1, OR with the previous bit 7
    ...
    case 0x09: // DAD B : HL = HL + BC (add register pair BC to register pair HL). only the CY flag is affected.
    {
        ...
        uint32_t value = (uint16_t)state->b << 8 | (uint16_t)state->c;
    ...
    case 0xce: // ACI D8 - add immeidate with carry.
    {
