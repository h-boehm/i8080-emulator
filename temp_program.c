// all our code in one file
// compile with gcc -o temp_program temp_program.c
...
{
    // pc will have advanced 1, so undo that
...
{
    // decrement stack pointer, push high byte onto stack
    state->memory[state->sp - 1] = high_byte;
    // decrement stack pointer, push low byte onto stack
    state->memory[state->sp - 2] = low_byte;
    // decrement sp
    state->sp -= 2;
}

int get_parity(int x, int size)
{
    int p = 0;
    x = (x & ((1 << size) - 1));
...
// disassmbler
int disassemble_8080(unsigned char *memory, int pc)
{
...
int emulate_8080(State8080 *state)
{
    ...
    case 0x04: // INR B
    {
        ...
        state->cc.p = get_parity(state->b, 8);
    ...
    case 0x0C: // INR C
    {
        ...
        state->cc.p = get_parity(state->c, 8);
    ...
    case 0x14: // INR D
        ...
        state->cc.p = get_parity(state->d, 8);
    ...
    case 0x1C: // INR E
        ...
        state->cc.p = get_parity(state->e, 8);
    ...
    case 0x22: // SHLD adr
    {
        uint16_t address = state->memory[state->pc] | (state->memory[state->pc + 1] << 8);
    ...
    case 0x24: // INR H
        ...
        state->cc.p = get_parity(state->h, 8);
    ...
    case 0x27: // DAA
    {
        uint8_t l = state->a & 0xf; // lower 4 bits
        if (l > 9 || state->cc.ac) {
            state->a += 6;
            state->cc.ac = 1;
        }
        ...
        state->cc.z = (state->a == 0);             // set zero flag if a is zero
        state->cc.s = (0x80 == (state->a & 0x80)); // set sign flag if bit 7 of a is set
        state->cc.p = get_parity(state->a, 8);     // set parity flag according to parity of a
        break;
    }
    case 0x29: // DAD H
    {
    ...
    case 0x2A: // LHLD adr
    {
        uint16_t adr = (state->memory[state->pc + 2] << 8) // fetch the 16-bit address
        | state->memory[state->pc + 1];
        state->l = state->memory[adr];                     // L <- M[adr]
        state->h = state->memory[adr + 1];                 // H <- M[adr+1]
        state->pc += 2;
        break;
    }
    case 0x2B: // DCX H
    ...
    case 0x2C: // INR L
        ...
        state->cc.p = get_parity(state->l, 8);
    ...
    case 0x34: // INR M
    {
        ...
        state->cc.p = get_parity(state->memory[offset], 8);
    ...
    case 0x3C: // INR A
        ...
        state->cc.p = get_parity(state->a, 8);
    ...
    case 0x80: // ADD B
    {
        ...
        if (answer > 0xff)
            ...
            state->cc.cy = 0;
        // get_parity is handled by a subroutine
        state->cc.p = get_parity(answer & 0xff, 8);
        // auxiliary carry
    ...
    case 0xB8: // CMP B
    {
        ...
        state->cc.p = get_parity(res & 0xff, 8);
        state->cc.ac = (((state->a & 0xf) + (-(state->c) & 0xf)) > 0xf);
        break;
    }
    case 0xB9: // CMP C
    ...
    case 0xE6: // ANI byte
    {
        ...
        state->cc.p = get_parity(x, 8);
        state->cc.cy = 0; // datasheet says op clears carry flags
        state->cc.ac = 0;
        state->a = x;
        state->pc++;
        break;
